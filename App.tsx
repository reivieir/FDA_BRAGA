...
const App = () => {
      ...
      <div className="min-h-screen bg-[#FDFCF0] p-6 font-sans">
        ...
        <div className="max-w-xl mx-auto space-y-6">
          <div className="bg-black text-white p-8 rounded-[40px] shadow-xl border-b-8 border-[#D4A373]">
            <p className="text-[10px] text-[#D4A373] font-black uppercase tracking-widest italic mb-2">Relatório Mensal</p>
            <h2 className="text-4xl font-black uppercase italic">{selectedMonth}</h2>
            <div className="mt-4 text-2xl font-black text-green-500">R$ {totalMes.toLocaleString('pt-BR')}</div>
          </div>

          <div className="space-y-3">
            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">Membros que pagaram ({pagamentosDoMes.length})</h3>
            {pagamentosDoMes.map(p => (
              <div key={p.id} className="bg-white p-5 rounded-3xl shadow-sm flex justify-between items-center border border-gray-100">
            ...
            {pagamentosDoMes.length === 0 && <p className="text-center text-gray-400 italic py-10">Nenhum pagamento este mês.</p>}
          </div>
        </div>
      </div>
    );
  ...
  // --- TELA DETALHES MEMBRO (COM REGRA DIA 15) ---
  if (selectedMembroId) {
    ...
    return (
      <div className="min-h-screen bg-[#FDFCF0] p-6 font-sans">
        ...
        <div className="max-w-xl mx-auto bg-white p-8 rounded-[40px] shadow-xl border-b-8 border-green-700">
          ...
          <div className={`mt-4 p-4 rounded-2xl text-center font-black uppercase text-xs italic ${pagouMesAtual ? 'bg-green-100 text-green-700' : posPrazo ? 'bg-red-100 text-red-600' : 'bg-blue-50 text-blue-500'}`}>
            {pagouMesAtual ? "✨ parabens, até que em fim pagou" : posPrazo ? "⚠️ Paga o que deve caloteiro" : "⏳ Aguardando Pix até dia 15"}
          </div>
          <div className="flex justify-between mt-8 text-sm font-bold">
            <span className="text-green-600 font-black text-xl">R$ {pago}</span>
            <span className="text-gray-400 pt-2 uppercase tracking-tighter italic">FALTA R$ {Math.max(0, meta - pago)}</span>
          </div>
          ...
          <div className="mt-8 space-y-3">
            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">Extrato</h3>
            {pagamentos.map(p => (
              <div key={p.id} className="flex justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 shadow-sm">
                <span className="font-bold text-gray-600 uppercase italic text-xs">{p.mes}</span>
                <span className="font-black text-green-600">R$ {p.valor}</span>
              </div>
            ))}
          </div>
    ...
    return (
      <div className="p-4 bg-gray-100 min-h-screen font-sans pb-20">
        <div className="flex justify-between items-center mb-6 max-w-2xl mx-auto">
           <button onClick={() => setIsAdmin(false)} className="text-blue-600 font-bold text-xs uppercase tracking-widest">← Painel</button>
          ...
          {meses.map(mes => {
            ...
            return sum > 0 ? (
              <div 
                ...
                className="bg-gray-900/50 p-3 rounded-2xl text-center border border-gray-800 cursor-pointer hover:bg-gray-800 transition-all hover:scale-105 active:scale-95"
              >
