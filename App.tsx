...
const App = () => {
  ...
  const metaMensalGrupo = 1590; // (17490 meta total / 11 meses)
          ...
          <div className="bg-black text-white p-8 rounded-[40px] shadow-xl border-b-8 border-[#D4A373]">
            ...
            <div className="mt-4 flex justify-between items-end">
                <div>
                    ...
                    <p className="text-2xl font-black text-green-500">R$ {pagsMes.reduce((acc, p) => acc + Number(p.valor), 0).toLocaleString('pt-BR')}</p>
                </div>
                <div className="text-right">
                    <p className="text-[10px] text-gray-500 font-black uppercase">Meta do Mês</p>
                    <p className="text-sm font-bold text-gray-400 italic">R$ {metaMensalGrupo}</p>
                </div>
            </div>
          </div>
          <div className="space-y-3">
            {pagsMes.map(p => (
              <div key={p.id} className="bg-white p-5 rounded-3xl shadow-sm flex justify-between border border-gray-100 italic">
                <span className="font-black text-gray-700 uppercase text-sm">{p.membros?.nome}</span>
      ...
      <div className="min-h-screen bg-[#FDFCF0] p-6 font-sans">
        ...
        <div className="max-w-xl mx-auto bg-white p-8 rounded-[40px] shadow-xl border-b-8 border-green-700">
          ...
          <div className="flex justify-between mt-8 text-sm font-bold">
            ...
            <span className="text-gray-400 pt-2 uppercase italic tracking-tighter">META R$ {meta}</span>
          </div>
                  ...
                  if (filtrosGrupos[idx] === 'Todos') {
                    ...
                    return historico.filter(h => h.membro_id === m.id).map(p => (
                      <div key={p.id} className="flex justify-between items-center p-3 bg-red-50/30 rounded-xl border border-red-100">
                        ...
                        <button onClick={() => excluirPagamento(p.id)} className="text-red-500 font-black text-[10px] uppercase tracking-tighter">Excluir</button>
                      </div>
                    ));
                  }
                })}
    ...
    <div className="min-h-screen bg-[#FDFCF0] p-4 md:p-10 font-sans text-gray-800">
      ...
      <div className="max-w-6xl mx-auto bg-black text-white p-8 rounded-[40px] shadow-2xl mb-12 border-b-8 border-green-700">
        <div className="flex justify-between items-center mb-8 gap-4">
          ...
          <div className="text-right">
            <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest italic">Meta Final</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 border-t border-gray-800 pt-6">
          {meses.map(mes => {
            ...
            return sum > 0 ? (
              <div key={mes} onClick={() => setSelectedMonth(mes)} className="bg-gray-900/50 p-3 rounded-2xl border border-gray-800 cursor-pointer hover:bg-gray-800 transition-all">
                <p className="text-[8px] font-black text-gray-500 uppercase tracking-tighter">{mes}</p>
                <p className="text-sm font-black text-[#D4A373]">R$ {sum}</p>
                <p className="text-[7px] font-bold text-gray-600 uppercase mt-1">Meta R$ {metaMensalGrupo}</p>
              </div>
            ) : null;
          })}
        </div>
              ...
              return (
                <div key={nome} onClick={() => m && setSelectedMembroId(m.id)} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 cursor-pointer hover:scale-[1.03] transition-all group active:scale-95">
                  ...
                  <div className="flex justify-between text-[10px] font-black italic tracking-tighter">
                    <span className="text-gray-300 uppercase">Falta R$ {Math.max(0, meta - pago)}</span>
