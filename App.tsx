...
const App = () => {
  ...
  // Mapeamento para Display vs Banco de Dados
  ...
  const mesesFull = Object.values(mesesMap);
          ...
          <div className="bg-black text-white p-8 rounded-[40px] shadow-xl border-b-8 border-red-500 text-center italic">
            ...
            <div className="mt-4 border-t border-gray-800 pt-4 flex justify-between items-end italic">
              <div className="text-left">
                ...
                <p className="text-[11px] text-[#D4A373] font-black uppercase mt-1 tracking-tighter">{pagantesCount} de 27 pagaram</p>
              </div>
  ...
  if (selectedMembroId) {
    ...
    return (
      <div className="min-h-screen bg-[#FDFCF0] p-6 font-sans text-gray-800">
        <button onClick={() => setSelectedMembroId(null)} className="mb-8 font-black text-[#D4A373] uppercase text-xs tracking-widest italic">← Voltar</button>
        <div className="max-w-xl mx-auto bg-white p-8 rounded-[40px] shadow-xl border-b-8 border-green-700 italic">
          ...
          <div className="flex justify-between mt-8 text-sm font-bold italic">
            ...
            <span className="text-gray-400 pt-2 uppercase text-xs tracking-tighter italic">Meta Natal R$ {getMetaInd(m?.nome || '')}</span>
          </div>
      ...
      <div className="p-4 bg-gray-100 min-h-screen font-sans pb-20 text-gray-800 italic">
        <div className="flex justify-between items-center mb-6 max-w-2xl mx-auto italic">
           ...
           <select className="p-2 border rounded-xl text-xs font-bold bg-white" value={mesGlobal} onChange={e => setMesGlobal(e.target.value)}>
             {mesesFull.map(m => <option key={m} value={m}>{m}</option>)}
           </select>
        </div>
        <div className="space-y-6 max-w-2xl mx-auto">
          {/* UPLOAD */}
          <div className="bg-blue-900 text-white p-6 rounded-3xl shadow-lg border-b-4 border-blue-400 italic">
            <h2 className="text-[10px] font-black uppercase mb-4 tracking-widest italic">1. Auditoria</h2>
            <div className="flex gap-2 mb-4 italic">
              <input type="text" placeholder="Ex: Extrato Fev" className="flex-1 p-3 rounded-xl text-black text-xs" value={nomeDoc} onChange={e => setNomeDoc(e.target.value)} />
                  ...
                  if (filtrosGrupos[idx] === 'Todos') {
                    ...
                    return historico.filter(h => h.membro_id === m.id).map(p => (
                      <div key={p.id} className="flex justify-between items-center p-3 bg-red-50/30 rounded-xl border border-red-100 italic">
                        ...
                        <button onClick={() => { if (confirm("Excluir?")) { supabase.from('pagamentos_detalhes').delete().eq('id', p.id).then(fetchAll); } }} className="text-red-500 font-black text-[10px] uppercase italic">Excluir</button>
                      </div>
                    ));
                  }
                })}
    ...
    <div className="min-h-screen bg-[#0B0C10] p-4 md:p-8 font-sans text-white">
      ...
      <div className="max-w-4xl mx-auto bg-[#121418] rounded-[40px] p-6 md:p-8 shadow-2xl border border-gray-800 mb-8 italic">
        <div className="flex flex-col md:flex-row justify-between gap-6 mb-8 italic">
          ...
          <div className="bg-[#1A1D23] p-4 rounded-3xl border border-gray-800 self-start md:self-center italic">
            ...
            <p className="text-xl font-black italic italic">R$ {totalArrecadado.toLocaleString('pt-BR')}</p>
          </div>
        </div>
        ...
        <div className="mb-8 italic">
           <div className="flex justify-between items-end mb-2 italic">
              <div>
                ...
                <p className="text-xl font-black italic italic">R$ {metaGlobalBragança.toLocaleString('pt-BR')}</p>
              </div>
              <p className="text-xl font-black text-[#22c55e] italic italic">{( (totalArrecadado/metaGlobalBragança)*100 ).toFixed(1)}%</p>
           </div>
        ...
        {/* TABELA ULTRA-CLEAN PARA SMARTPHONE */}
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 pb-24 italic">
        <div className="space-y-4 italic">
          <h2 className="text-[9px] font-black text-gray-600 uppercase tracking-widest ml-4 italic">Membros</h2>
          {gruposDef.map((g, gIdx) => (
            <div key={gIdx} className="bg-[#121418] rounded-3xl border border-gray-800 overflow-hidden italic shadow-sm">
                    ...
                    return (
                      <div key={nome} onClick={() => m && setSelectedMembroId(m.id)} className="bg-[#1A1D23] p-3 rounded-xl flex justify-between items-center border border-gray-800 italic">
                        ...
                        <div className="flex items-center gap-2 italic">
                          ...
                          <div className={`h-1 w-1 rounded-full italic ${pagoMembro >= metaMembro ? 'bg-[#22c55e]' : 'bg-red-900'}`}></div>
                        </div>
                      </div>
                    );
                  })}
        ...
        <div className="space-y-4 italic">
          <h2 className="text-[9px] font-black text-gray-600 uppercase tracking-widest ml-4 italic">Saídas Detalhadas</h2>
      ...
      <footer className="mt-6 text-center pb-20 pt-6 border-t border-dashed border-gray-800 italic italic">
        ...
        <button onClick={() => senha === 'FDA2026' && setIsAdmin(true)} className="ml-2 text-[9px] font-black text-gray-600 uppercase tracking-widest italic">Acessar</button>
      </footer>
    </div>
  );
};
