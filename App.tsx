...
        <div className="max-w-xl mx-auto bg-black text-white p-8 rounded-[40px] shadow-xl border-b-8 border-red-500 text-center italic">
...
          <h2 className="text-3xl font-black italic uppercase text-gray-800 tracking-tighter italic">{m?.nome}</h2>
...
            <span className="text-green-600 font-black text-xl italic italic">R$ {pagoAcumulado}</span>
            <span className="text-gray-400 pt-2 uppercase text-xs tracking-tighter italic italic">Meta Natal R$ {metaMembro}</span>
          </div>
...
            <div className="flex gap-2 mb-4 italic italic">
              <input type="text" placeholder="Nome do arquivo" className="flex-1 p-3 rounded-xl text-black text-xs italic italic" value={nomeDoc} onChange={e => setNomeDoc(e.target.value)} />
              <input type="file" className="w-24 text-[8px] pt-3 italic italic" onChange={e => setFileToUpload(e.target.files?.[0] || null)} />
              <button onClick={handleUpload} className="bg-blue-500 px-4 rounded-xl text-[10px] font-black italic italic">SUBIR</button>
            </div>
            {docs.map(d => (
                    ...
                    return (
                      <tr key={mesAbbr} onClick={() => setSelectedMonth(mesAbbr)} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-all cursor-pointer italic">
                        ...
                        <td className="py-4 text-green-500 italic">
                          {arrec > 0 ? `R$ ${arrec.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}` : '—'}
                        </td>
                        <td className="py-4 text-red-400 italic">
                          {saidaM > 0 ? `R$ ${saidaM.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}` : '—'}
                        </td>
