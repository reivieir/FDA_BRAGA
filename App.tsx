...
const App = () => {
  ...
  if (selectedMonth) {
    ...
    return (
      <div className="min-h-screen bg-[#FDFCF0] p-6 font-sans text-gray-800 italic">
        ...
        <div className="max-w-xl mx-auto bg-black text-white p-8 rounded-[40px] shadow-xl border-b-8 border-red-500 italic text-center">
  ...
  // TELA 2: DETALHE DO MEMBRO (META PESSOAL)
  if (selectedMembroId) {
    ...
    return (
      <div className="min-h-screen bg-[#FDFCF0] p-6 font-sans text-gray-800 italic">
        <button onClick={() => setSelectedMembroId(null)} className="mb-8 font-black text-[#D4A373] uppercase text-xs tracking-widest">← Voltar</button>
        <div className="max-w-xl mx-auto bg-white p-8 rounded-[40px] shadow-xl border-b-8 border-green-700">
          ...
          <div className="flex justify-between mt-8 text-sm font-bold italic">
            <span className="text-green-600 font-black text-xl italic">R$ {pagoAcumulado}</span>
            <span className="text-gray-400 pt-2 uppercase text-xs tracking-tighter italic">Meta Total R$ {metaMembro}</span>
          </div>
          <div className="w-full bg-gray-100 h-2 rounded-full mt-2 overflow-hidden italic">
             <div className="bg-green-500 h-full transition-all duration-1000" style={{ width: `${Math.min((pagoAcumulado/metaMembro)*100, 100)}%` }}></div>
          </div>
          <div className="mt-8 space-y-3 italic">
            {pags.map(p => (
              <div key={p.id} className="flex justify-between p-4 bg-gray-50 rounded-2xl border italic text-xs">
                ...
                <span className="font-black text-green-600 italic">R$ {p.valor}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="p-4 bg-gray-100 min-h-screen font-sans pb-20 text-gray-800 italic">
        ...
        <div className="space-y-6 max-w-2xl mx-auto italic">
          <div className="bg-blue-900 text-white p-6 rounded-3xl shadow-lg border-b-4 border-blue-400 italic">
            <h2 className="text-[10px] font-black uppercase mb-4 tracking-widest italic">1. Auditoria e Comprovantes</h2>
            <div className="flex gap-2 mb-4 italic">
              <input type="text" placeholder="Nome do arquivo" className="flex-1 p-3 rounded-xl text-black text-xs italic" value={nomeDoc} onChange={e => setNomeDoc(e.target.value)} />
              <input type="file" className="w-24 text-[8px] pt-3 italic" onChange={e => setFileToUpload(e.target.files?.[0] || null)} />
            ...
            {docs.map(d => (
              <div key={d.id} className="flex justify-between items-center bg-blue-800/30 p-2 rounded-lg text-[9px] mb-2 italic">
                ...
                <button onClick={() => excluirDoc(d.id, d.url_arquivo)} className="text-red-400 font-black italic">X</button>
              </div>
            ))}
          </div>
          <div className="bg-black text-white p-6 rounded-3xl shadow-lg border-b-4 border-red-500 italic">
            <h2 className="text-[10px] font-black uppercase mb-4 tracking-widest italic">2. Lançar Gasto</h2>
            ...
            <div className="mt-4 space-y-1 border-t border-gray-800 pt-3">
               {saidas.slice(0, 5).map(s => (
                 <div key={s.id} className="flex justify-between items-center text-[9px] italic">
                   ...
                   <button onClick={() => excluirItem(s.id, 'saidas_caixa')} className="text-red-500 italic">X</button>
                 </div>
               ))}
            </div>
          </div>
    ...
    <div className="min-h-screen bg-[#0B0C10] p-4 md:p-8 font-sans text-white italic">
      ...
      <div className="max-w-4xl mx-auto bg-[#121418] rounded-[40px] p-6 md:p-8 shadow-2xl border border-gray-800 mb-8 italic">
        <div className="flex flex-col md:flex-row justify-between gap-6 mb-8 italic">
          ...
          <div className="bg-[#1A1D23] p-4 rounded-3xl border border-gray-800 self-start md:self-center italic shadow-inner">
        ...
        <div className="mb-8 italic">
           <div className="flex justify-between items-end mb-2 italic">
              <div>
                <p className="text-[9px] text-gray-500 font-black uppercase italic">META GLOBAL BRAGANÇA</p>
                 ...
                 {mesesAbbr.map(mesAbbr => {
                   ...
                   return (
                     <tr key={mesAbbr} onClick={() => setSelectedMonth(mesAbbr)} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-all cursor-pointer italic">
                       ...
                       <td className="py-4 text-red-400 italic">{saidaMes > 0 ? `R$ ${saidaMes}` : '—'}</td>
      ...
      {/* SEÇÃO MEMBROS (CLIQUE RESTAURADO) */}
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 pb-24 italic">
        <div className="space-y-4 italic">
          <h2 className="text-[9px] font-black text-gray-600 uppercase tracking-widest ml-4 italic">Membros e Metas Pessoais</h2>
            ...
            <div key={gIdx} className="bg-[#121418] rounded-3xl border border-gray-800 overflow-hidden shadow-sm italic">
              <button onClick={() => setExpandedGrupo(expandedGrupo === gIdx ? null : gIdx)} className="w-full p-4 flex justify-between items-center hover:bg-gray-800 transition-all italic">
                <div className="text-left italic">
                  ...
                  <p className="text-[7px] font-black text-gray-600 uppercase mt-1 italic italic">
              ...
              <div className={`transition-all duration-300 ${expandedGrupo === gIdx ? 'max-h-[800px] p-4' : 'max-h-0'} overflow-hidden italic`}>
                <div className="grid grid-cols-1 gap-2 italic">
                  {g.nomes.map(nome => {
                    const m = membros.find(x => x.nome === nome);
                    const pagoMembro = m ? calcPago(m.id) : 0;
                    const metaMembro = getMetaInd(nome);
                    return (
                      <div key={nome} onClick={() => m && setSelectedMembroId(m.id)} className="bg-[#1A1D23] p-3 rounded-xl flex justify-between items-center border border-gray-800 hover:bg-gray-800 cursor-pointer transition-all italic">
                        <span className="font-black text-[9px] uppercase text-gray-500 italic">{nome}</span>
                        <div className="flex items-center gap-2 italic">
                          <span className="text-[9px] font-black text-[#22c55e] italic">R$ {pagoMembro}</span>
        ...
        {/* AUDITORIA E EXTRATOS RESTAURADOS */}
        <div className="space-y-4 italic">
          <h2 className="text-[9px] font-black text-blue-900 uppercase tracking-widest ml-4 italic">Auditoria (Comprovantes)</h2>
          <div className="bg-blue-950/10 rounded-[30px] p-6 border border-blue-900/30 min-h-[120px] italic">
            {docs.map(d => (
              <a key={d.id} href={d.url_arquivo} download target="_blank" className="flex justify-between items-center p-3 mb-2 bg-[#121418] rounded-xl border border-gray-800 italic hover:bg-blue-900/20 transition-all">
        ...
        {/* SAÍDAS DETALHADAS */}
        <div className="space-y-4 italic">
          <h2 className="text-[9px] font-black text-red-900 uppercase tracking-widest ml-4 italic">Resumo de Saídas</h2>
          <div className="bg-[#121418] rounded-[30px] p-6 border border-gray-800 min-h-[120px] italic">
            {saidas.slice(0, 5).map(s => (
              <div key={s.id} className="mb-3 border-b border-gray-800 pb-3 last:border-0 italic">
                <div className="flex justify-between items-start mb-1 text-[7px] font-black italic italic">
      ...
      <footer className="mt-6 text-center pb-20 pt-6 border-t border-dashed border-gray-800 italic italic">
        ...
        <button onClick={() => senha === 'FDA2026' && setIsAdmin(true)} className="ml-2 text-[9px] font-black text-gray-600 uppercase tracking-widest italic">Acesso Admin</button>
      </footer>
    </div>
  );
};
